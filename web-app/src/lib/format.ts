...
/**
 * Returns a human-readable due date label relative to today.
 *
 * For strict ISO dates (YYYY-MM-DD):
 *   - Today   → "Due today" / "今日到期"
 *   - Future  → "Due in N days" / "N 天后到期"
 *   - Overdue → "Overdue by N days" / "逾期 N 天"
 *
 * Legacy loose strings ("today", "Fri", "next wk") are passed through for
 * backward-compat with pre-migration data that bypassed the backend.
 */
export function getDueLabel(due: string | null, locale: Locale): string | null {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(due)) {
    ...
    // Calculate calendar-day difference using local midnight to avoid DST drift
