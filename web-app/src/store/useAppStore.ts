...
interface AppState {
  ...
  /** Daily focus capacity in minutes. Default 360 (6h). Range 60–720 (1–12h). */
  dailyFocusCapacityMinutes: number;
  ...
  setDailyFocusCapacityMinutes: (m: number) => void;
}
...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      setDailyFocusCapacityMinutes: (dailyFocusCapacityMinutes) => set({ dailyFocusCapacityMinutes }),
    }),
    { name: "lumo.app.v1" }
  )
);
